...
(defn find-style-diffs [acc coord old-style new-style]
  (if (identical? old-style new-style)
    ...
    (cond
      ...
      (and (identical? 0 (count old-style)) (> (count new-style) 0)) (let [entry (get
                                                                                   new-style
                                                                                   0)
                                                                           follows (subvec
                                                                                     new-style
                                                                       ...
                                                                       (recur
                                                                         ...
                                                                         old-style
                                                                         follows))
      (and (> (count old-style) 0) (identical? 0 (count new-style))) (let [entry (get
                                                                                   old-style
                                                                                   0)
                                                                           follows (subvec
                                                                                     old-style
      ...
      :else (let [old-entry (get old-style 0)
              ...
              (case
                ...
                (recur
                  ...
                  new-style)
                1
                (recur (conj acc [:add-style coord new-entry]) coord old-style new-follows)
                (recur
  ...
  (cond
    ...
    :else (let [old-entry (get old-props 0)
            ...
            (case
              (compare old-k new-k)
              -1
              (recur (conj acc [:rm-prop coord old-k]) coord old-follows new-props)
              1
              (recur (conj acc [:add-prop coord new-entry]) coord old-props new-follows)
              (recur
...
(defn find-children-diffs [acc n-coord index old-children new-children]
  ...
  (let [old-size (count old-children) new-size (count new-children)]
    (cond
      ...
      (and (zero? old-size) (pos? new-size)) (let [element (last (first new-children))
                                                   next-acc (conj
                                                              acc
                                                              [:append
                                                               n-coord
      ...
      (and (pos? old-size) (zero? new-size)) (let [next-acc (conj
                                                              acc
      ...
      :else (let [old-keys (mapv first old-children)
              ...
              (cond
                ...
                (and x1-remains? (not y1-existed?)) (let [next-acc (conj
                                                                     acc
                                                                     (let [element (last
                                                                                     (first
                                                                                       new-children))]
                                                      ...
                                                      (recur
                                                        ...
                                                        old-children
                                                        new-follows))
                (and (not x1-remains?) y1-existed?) (let [next-acc (conj
                                                                     acc
                                                                     [:rm
                                                                      (conj
                                                                        n-coord
...
(defn find-element-diffs [acc n-coord old-tree new-tree]
  ...
  (if (identical? old-tree new-tree)
    ...
    (cond
      ...
      :else (let [old-children (:children old-tree) new-children (:children new-tree)]
              (if (or
